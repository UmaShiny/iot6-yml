<script type="text/html" data-help-name="e-mail">
    ...
    default value.</p>
	<p> You can also set <code> msg.host, msg.userid, msg.password </code> in the payload, which will override <code> server, username and password </code>
     default values. For example, if the data is stored in a database or elsewhere. </p>
	<p> Optionally, you can set <code> msg.port </code> in the payload, which will override <code> port </code>
     default value. </p>
    <h3>Gmail users</h3>
