    ...
    function EmailNode(n) {
        ...
        this.on("input", function(msg, send, done) {
            if (msg.hasOwnProperty("payload")) {
				...
				if(msg.password && msg.userid && msg.host){
					smtpOptions.host = msg.host ? msg.host : node.outserver, // NEW
					smtpOptions.port = msg.port ? msg.port : node.outport // NEW
					smtpOptions.secure = msg.secure ? msg.secure : node.secure // NEW
					smtpOptions.auth.user = msg.userid ? msg.userid : node.userid, // NEW
					smtpOptions.auth.pass = msg.password ? msg.password : node.password // NEW
                ...
                if (smtpTransport) {
					...
					// add block start  //NEW
					var send_from = node.userid;					
					if(msg.from){
						send_from = msg.from;
					}
					else if(msg.userid){
						send_from = msg.userid;
					}
                    var sendopts = { from: send_from };   // sender address
					// add block end
                    ...
                    if (Buffer.isBuffer(msg.payload)) { // if it's a buffer in the payload then auto create an attachment instead
						...
						if (msg.hasOwnProperty("amp")) {
                            sendopts.amp = RED.util.ensureString(msg.amp);
                        }
						// helper for setting List-* headers (see more here https://nodemailer.com/message/list-headers/)
						if (msg.hasOwnProperty("list")) {
							if(util.isObject(msg.list)){
								sendopts.list = msg.list;
							}
                        }
						// optional Message-Id value, random value will be generated if not set
						if (msg.hasOwnProperty("messageId")) {
							sendopts.messageId = RED.util.ensureString(msg.messageId);
                        }
						...
						if (msg.hasOwnProperty("watchHtml")) {
							sendopts.watchHtml = RED.util.ensureString(msg.watchHtml);
                        }
						...
						if (msg.hasOwnProperty("encoding")) {
							sendopts.encoding = RED.util.ensureString(msg.encoding);
                        }
						// force content-transfer-encoding for text values (either quoted-printable or base64). 
						// By default the best option is detected (for lots of ascii use quoted-printable, otherwise base64)
						if (msg.hasOwnProperty("textEncoding")) {
							sendopts.textEncoding = RED.util.ensureString(textEncoding);
                        }
						// optional Date value, current UTC string will be used if not set
						if (msg.hasOwnProperty("date")) {
							sendopts.date = RED.util.ensureString(date);
                        }
						// Routing options
						...
						if (msg.hasOwnProperty("sender")) {
							sendopts.sender = RED.util.ensureString(sender);
                        }
						//  An email address that will appear on the Reply-To: field
						if (msg.hasOwnProperty("replyTo")) {
							sendopts.replyTo = RED.util.ensureString(replyTo);
                        }
						// The Message-ID this message is replying to
						if (msg.hasOwnProperty("inReplyTo")) {
							sendopts.inReplyTo = RED.util.ensureString(inReplyTo);
                        }
						// Message-ID list (space separated string)
						if (msg.hasOwnProperty("references")) {
							sendopts.references = RED.util.ensureString(references);
                        }
						//  iCalendar event to use as an alternative. See details https://nodemailer.com/message/calendar-events/
						if (msg.hasOwnProperty("icalEvent")) {
							if(util.isObject(icalEvent)){
								sendopts.icalEvent = RED.util.getObjectProperty(msg.icalEvent);
							}
                        }
						////////// end add new logic //////////
                        if (msg.attachments) {
                            ...
                            for (var a=0; a < sendopts.attachments.length; a++) {
                                if (sendopts.attachments[a].hasOwnProperty("content")) {
                                    if (typeof sendopts.attachments[a].content !== "string" && !Buffer.isBuffer(sendopts.attachments[a].content)) {
										...
										if (node.output) {	// NEW	 
											msg.payload = {success: false, error: 104, eerror_text: "invalidattachment"};
											node.send(msg); 
										}
                                        return;
                                    }
                    ...
                    smtpTransport.sendMail(sendopts, function(error, info) {
                        if (error) {
							...
							if (node.output) {	// NEW	 
								msg.payload = {success: false, error: 101, error_text: error};
								node.send(msg); 
							}
                        } else {
                            ...
                            if (done) { 
								if (node.output) {	// NEW										
									msg.payload = {success: true, response: info};
									node.send(msg); 
								}
								done(); 
							}
					...
					if (node.output) {	// NEW	 
						msg.payload = {success: false, error: 102, error_text: "nosmtptransport"};
						node.send(msg); 
					}
				}
            }
				...
				if (node.output) {	// NEW	 
					msg.payload = {success: false,  error: 103, error_text: "nopayload"};
					node.send(msg); 
				}
			}			
        });
